...
pthread_mutex_t mLock; // THREAD CHANGE: allocator now uses pthread_mutex instead of process semaphores
...
void *init_umem(void) {
    // THREAD CHANGE: threads share the same process heap, so allocator setup remains process-local rather than cross-process shared memory
...
void *umalloc(size_t size) {
    if (use_multithread) pthread_mutex_lock(&mLock); // THREAD CHANGE: mutex lock replaces sem_wait on shared semaphore
    void* p = _umalloc(size);
    if (use_multithread) pthread_mutex_unlock(&mLock); // THREAD CHANGE: mutex unlock replaces sem_post
    return p;
}

void ufree(void *ptr) {
    if (use_multithread) pthread_mutex_lock(&mLock); // THREAD CHANGE: allocator critical section guarded by pthread mutex
    _ufree(ptr);
...
int main(int argc, char *argv[]) {
    ...
    use_multithread = (argc >= 3 && strcmp(argv[2], "-t") == 0); // THREAD CHANGE: -t flag selects threaded execution

    init_umem();

    if (use_multithread)
        return run_multi(filename); // THREAD CHANGE: dispatch to pthread-based run path
    else
...
} thread_task_t; // THREAD CHANGE: thread arguments replace per-process pipe payloads

static void *thread_worker(void *arg) {
    thread_task_t *task = (thread_task_t *)arg; // THREAD CHANGE: worker thread routine handles each block
    unsigned long h = process_block(task->buf, task->len);
    task->results[task->index] = h; // THREAD CHANGE: store block hash in shared results array
    ufree(task->buf);
    return NULL;
}
...
int run_multi(const char *filename) { // THREAD CHANGE: run_multi rewritten to use pthreads instead of fork/pipe logic
    FILE *fp = fopen(filename, "rb");
    if (!fp) {
        perror("fopen");
        return 1;
    ...
    unsigned char buf[BLOCK_SIZE];
    unsigned long final_hash = 0;
    pthread_t threads[1024]; // THREAD CHANGE: track pthreads instead of child PIDs
    thread_task_t tasks[1024];
    unsigned long results[1024] = {0}; // THREAD CHANGE: shared array replaces pipe buffers
    int num_blocks = 0;
    int join_cursor = 0;
    int active = 0;
    ...
    while (!feof(fp)) {
        ...
        if (pthread_create(&threads[num_blocks], NULL, thread_worker, &tasks[num_blocks]) != 0) { // THREAD CHANGE: replaced fork() with pthread_create()
        ...
        if (active == MAX_THREADS) {
            pthread_join(threads[join_cursor], NULL); // THREAD CHANGE: join replaces waitpid on child processes
            join_cursor++;
            active--;
        }
    }
    ...
    for (int i = join_cursor; i < num_blocks; i++)
        pthread_join(threads[i], NULL); // THREAD CHANGE: ensure all worker threads complete (no waitpid)
