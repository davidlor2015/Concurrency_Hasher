...
/*
 * PART 2 OPTIMIZED BUILD:
 * This version experiments with Strategy A (per-thread memory pools) and is
 * meant to be compared to sharedhash.c using:
 *
 *   gcc -pthread -Wall -o sharedhash sharedhash.c
 *   gcc -pthread -Wall -o esharedhash esharedhash.c
...
/* Per-thread small-block cache to reduce allocator mutex contention. */
#define LOCAL_CACHE_MAX 512
#define LOCAL_CACHE_LIMIT 4

typedef struct
{
    node_t *head;
    int count;
} local_cache_t; // OPTIMIZATION (Strategy A): per-thread memory pool metadata stored via TLS

static pthread_key_t cache_key;
static pthread_once_t cache_once = PTHREAD_ONCE_INIT;

static void destroy_cache(void *ptr);

static void init_cache_key(void)
{
    pthread_key_create(&cache_key, destroy_cache);
}

static local_cache_t *get_cache(void)
{
    pthread_once(&cache_once, init_cache_key);
    local_cache_t *cache = pthread_getspecific(cache_key); // OPTIMIZATION: map this thread to its dedicated cache via TLS lookup
    if (!cache)
    {
        cache = calloc(1, sizeof(local_cache_t)); // OPTIMIZATION (Strategy A): allocate a private free list head per thread
...
static atomic_int lock_count = 0; // METRIC: counts global lock acquisitions to quantify contention
...
static void flush_cache_to_global(local_cache_t *cache)
{
    if (!cache || !cache->head)
        return;
    if (use_multithread)
    {
        //  Global lock is still required here because cached blocks are merged back into the shared allocator state
        pthread_mutex_lock(&mLock);
        atomic_fetch_add_explicit(&lock_count, 1, memory_order_relaxed); // count global lock acquisitions while flushing shared state
    }
    node_t *curr = cache->head;
    while (curr)
    {
        node_t *next = curr->next;
        header_t *hdr = (header_t *)curr;
        hdr->magic = MAGIC;
        hdr->size = curr->size;
        _ufree((char *)curr + sizeof(header_t));
        curr = next;
    }
    if (use_multithread)
...
static void destroy_cache(void *ptr)
{
    local_cache_t *cache = (local_cache_t *)ptr;
    flush_cache_to_global(cache);
    free(cache);
}

static void *cache_alloc(size_t size)
{
    if (!use_multithread || size > LOCAL_CACHE_MAX)
        return NULL;
    local_cache_t *cache = get_cache(); // OPTIMIZATION (Strategy A)
    node_t *prev = NULL;
    node_t *curr = cache->head;
    size = ALIGN(size);
    while (curr)
...
static int cache_free(void *ptr)
{
    if (!use_multithread)
        return 0;
    header_t *hdr = (header_t *)((char *)ptr - sizeof(header_t));
    if (hdr->size > LOCAL_CACHE_MAX)
        return 0;
    local_cache_t *cache = get_cache(); // recycle frequently-used block sizes into the thread-local cache
    if (cache->count >= LOCAL_CACHE_LIMIT)
        return 0;
...
void *umalloc(size_t size)
{
    // FAST PATH: attempt to satisfy the allocation from this thread's local cache without taking the global lock.
    void *cached = cache_alloc(size);
    if (cached)
        return cached;
    // SLOW PATH: fall back to the shared allocator protected by the global mutex.
    if (use_multithread)
    {
        pthread_mutex_lock(&mLock);
        atomic_fetch_add_explicit(&lock_count, 1, memory_order_relaxed); // track slow-path lock acquisitions
    }
...
void ufree(void *ptr)
{
    ...
    // SLOW PATH: returning larger blocks to the shared allocator requires the global mutex.
    if (use_multithread)
    {
        pthread_mutex_lock(&mLock);
        atomic_fetch_add_explicit(&lock_count, 1, memory_order_relaxed); //  lock taken to modify shared free list
    }
    _ufree(ptr);
    if (use_multithread)
        pthread_mutex_unlock(&mLock);
...
int main(int argc, char *argv[])
{
    ...
    int rc;
    if (use_multithread)
        rc = run_multi(filename);
    else
        rc = run_single(filename);

    // REPORT: print lock acquisition count so we can compare contention with the baseline allocator.
    printf("Global lock acquisitions: %d\n", atomic_load_explicit(&lock_count, memory_order_relaxed));
    return rc;
}
 ...
 *
 * Threads share the same address space, so pipes are unnecessary. Each
 * block's hash is written into a shared results array, and the allocator
...
static void *thread_worker(void *arg)
{
    ...
    unsigned long h = process_block(task->buf, task->len);
    task->results[task->index] = h;
    ufree(task->buf);
    flush_cache_to_global(get_cache());
    return NULL;
}
...
int run_multi(const char *filename)
{
    ...
    thread_task_t tasks[1024];
    unsigned long results[1024] = {0};
    int num_blocks = 0;
    int join_cursor = 0;
    int active = 0;
    ...
    while (!feof(fp))
    {
        ...
        tasks[num_blocks].index = num_blocks;
        tasks[num_blocks].len = n;
        tasks[num_blocks].buf = block_buf;
        tasks[num_blocks].results = results;

        if (pthread_create(&threads[num_blocks], NULL, thread_worker, &tasks[num_blocks]) != 0)
        {
        ...
        {
            pthread_join(threads[join_cursor], NULL);
            join_cursor++;
            active--;
        }
    }
    ...
    for (int i = join_cursor; i < num_blocks; i++)
        pthread_join(threads[i], NULL);

    for (int i = 0; i < num_blocks; i++)
    {
        unsigned long h = results[i];
        print_intermediate(i, h, getpid());
        final_hash = (final_hash + h) % LARGE_PRIME;
    }
